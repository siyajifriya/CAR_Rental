...
// Alias export (Fix for your import error)
...
export const deleteBooking = async (req,res,next) => {
    ...
    // Assign guest bookings (no userId) to the authenticated user by email
    export const assignBookingsByEmail = async (req, res, next) => {
        try {
            ...
            // Safety: only allow assigning bookings that match the authenticated user's email
            if (String(req.user.email || '').trim().toLowerCase() !== email) {
                return res.status(403).json({ success: false, message: 'Email does not match authenticated user' });
            }

            const filter = { email: { $regex: `^${email}$`, $options: 'i' }, $or: [{ userId: { $exists: false } }, { userId: null }] };
