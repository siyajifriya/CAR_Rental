...
const api = axios.create({ baseURL: baseUrl, headers: { Accept: "application/json" } });
...
const NoBookingsView = ({onResetFilters}) => (
    <div className={BookingPageStyles.noBookingsContainer}>
        ...
        <h3 className={BookingPageStyles.noBookingsTitle}>No Booking Found</h3>
        <p className={BookingPageStyles.noBookingsText}>
            Try and adjusting your search or filter criteria.
        </p>
...
const Booking = () => {
  ...
  const updateStatus = async (id) => {
    ...
    setNewStatus('');
      ...
      console.error('Failed to updated status:', err);
      window.alert(
    ...
    <div className={BookingPageStyles.pageContainer}>
      ...
      <div className=" space-y-4">{
        filteredBookings.map((booking) => (
          <BookingCard key={booking.id} booking={booking} isExpanded={expandedBooking === booking.id}
          isEditing={editingStatus === booking.id} newStatus={newStatus} onToggleDetails={() => handleToggleDetails(booking.id)}
          onEditStatus={(e) => {
            e.stopPropagation();
          ...
          onCancelEdit={(e) => { e?.stopPropagation?.(); handleCancelEdit(); }}
          />
        ))}
