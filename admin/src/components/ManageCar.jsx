...
const StatCard = ({title, value, icon: Icon, className = ''}) => (
  <div className={`${styles.gradientOrange} ${styles.rounded2xl} ${styles.statCard} ${styles.borderGray} ${styles.borderHoverOrange}
  ${className}`}
  >
...
const EditModal = ({ car, onClose, onSubmit, onChange }) => {
  ...
  ); //MAP INPUT FIELD TO EDIT AND SEND IT UPDATED VERSION ON BACKEND 
};

const NoCarsView = ({onReset}) => (
  <div className={`${styles.gradientGray} ${styles.noCarsContainer}`}>
    <div className=" mx-auto w-24 h-24 rounded-full bg-gradient-to-br from-orange-900/30 to-amber-900/30 flex items-center
    justify-center mb-6"
    >
    ...
    <button onClick={onReset} className={`${styles.buttonPrimary} mt-4`}>
  Show All Cars
</button>
  </div>
);
