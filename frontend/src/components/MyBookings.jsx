        ...
        <div className={s.modalContent}>
          <div className="flex justify-between items-center mb-4">
            <h2 className={s.modalTitle}>
              <FaReceipt className="text-orange-400 inline mr-2" /> Booking Details
            </h2>
          ...
          {/* Car Image and Info Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <img src={booking.car.image} alt={booking.car.make} className={s.carImageModal} />
            </div>
            <div>
              <h3 className="text-lg font-semibold text-white mb-2">{booking.car.make}</h3>
              ...
              <div className="grid grid-cols-2 gap-3 mt-3">
                ...
                <div>
                  ...
                  <p className="text-base font-bold text-orange-400">{formatPrice(booking.price)}</p>
                </div>
              </div>
            </div>
          </div>

          {/* Booking Dates + Location Details (Left) | User Information + Payment Details (Right) */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
            {/* LEFT COLUMN */}
            <div className="space-y-6">
              {/* Booking Dates Card */}
              <div className="bg-gray-800/50 rounded-lg p-4 shadow-lg">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-start">
                    ...
                    <p className="text-base text-white font-medium">{formatDate(booking.dates.pickup)}</p>
                  </div>
                  <div className="flex justify-between items-start pb-3 border-b border-gray-700">
                    ...
                    <p className="text-base text-white font-medium">{formatDate(booking.dates.return)}</p>
                  </div>
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-sm text-gray-400">Duration:</p>
                    </div>
                    <p className="text-base text-white font-medium">{days} days</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-lg p-4 shadow-lg">
                ...
                <div>
                  ...
                  <p className="text-base text-white font-medium">{booking.location}</p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-gray-800/50 rounded-lg p-4 shadow-lg">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-base text-white font-medium">{booking.user.name}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Email:</p>
                    <p className="text-base text-white font-medium break-all">{booking.user.email}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Phone:</p>
                    <p className="text-base text-white font-medium">{booking.user.phone}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Address:</p>
                    <p className="text-base text-white font-medium">{booking.user.address}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gray-800/50 rounded-lg p-4 shadow-lg">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-base text-white font-medium">{booking.paymentMethod || "—"}</p>
                  </div>
                  <div>
                    <p className="text-sm text-gray-400 mb-1">Transaction ID:</p>
                    <p className="text-base text-white font-medium break-all">{booking.paymentId || booking.raw?.sessionId || "—"}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            <div>
              <p className="text-gray-400 text-xs mb-2">Booking Status:</p>
              <StatusBadge status={booking.status} />
            </div>
            <div>
              <p className="text-gray-400 text-xs mb-2">Booking Date:</p>
              <p className="text-white font-semibold">{formatDate(booking.bookingDate)}</p>
            </div>
          </div>

          {/* Close and Book Again Buttons */}
          <div className="flex justify-between gap-4 mt-6">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-6 py-3 bg-gray-700 text-white rounded-lg hover:bg-gray-800 font-semibold text-base"
            >
              Close
            </button>
            <Link
              to="/cars"
              onClick={onClose}
              className="flex-1 px-6 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 font-semibold text-base inline-flex items-center justify-center gap-2"
            >
...
const MyBookings = () => {
  ...
  // Claim guest bookings that match the logged-in user's email
  const claimBookings = async () => {
    try {
      const rawUser = localStorage.getItem('user');
      if (!rawUser) return alert('You must be logged in to claim bookings.');
      const user = JSON.parse(rawUser);
      const email = (user.email || '').trim();
      if (!email) return alert('Your account does not have an email.');
      if (!window.confirm(`Claim bookings that match ${email} and attach them to your account?`)) return;
      ...
      const res = await axios.post(`${API_BASE}/api/bookings/assign-mybookings`, { email }, { headers });
      if (res?.data?.success) {
        const count = res.data.modifiedCount ?? res.data.modified ?? 0;
        toast.success(`Claimed ${count} bookings. Refreshing...`, { autoClose: 2000 });
        // refetch
        fetchBookings();
      } else {
        toast.error(res?.data?.message || 'No bookings claimed');
      }
    } catch (err) {
      console.error('Claim bookings error', err);
      toast.error(err?.response?.data?.message || err.message || 'Failed to claim bookings');
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && filteredBookings.length === 0 && (
          <div className={s.emptyState}>
            ...
            {/* Claim bookings button: attempts to attach guest bookings matching logged-in email */}
            {localStorage.getItem('user') && (
              <div className="mt-4">
                <button
                  type="button"
                  onClick={claimBookings}
                  className="px-4 py-2 rounded-md bg-green-600 text-white"
                >
                  Claim Bookings (if any)
                </button>
              </div>
            )}
          </div>
